...
// File contracts/HungryCoin.sol

//SPDX-License-Identifier: MIT
pragma solidity ^0.8.0;
pragma experimental ABIEncoderV2;
contract HungryCoin is ERC20, Ownable {
  ...
  constructor() ERC20("HungryCoin", "HUNGRY") {
  ...
  function setMaxTransactionAmount(uint256 newNum) external onlyOwner {
    require(
      newNum >= totalSupply() / 10,
      "Max tx amount can't below 0.1% from total supply"
    );
  ...
  function _transfer(
    ...
    if (_decreasing && _limit > 0 && _perBlock > 0 && _percent > 0) {
      ...
      if (curBlockNumber.sub(_prevUpdatedBlock) > _perBlock) {
        uint256 deductAmount = curBlockNumber.sub(_prevUpdatedBlock).div(
          _perBlock
        ) * _percent;
    ...
    if (
      ...
      if (transferDelayEnabled) {
        if (
          ...
          uint256 distanceBlock = block.number -
